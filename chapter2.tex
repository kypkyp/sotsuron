...
% ここ自信ない
さらに、この中で直接話法を取る動詞は\textit{tok}、\textit{sei}の2つであるが、これは\textit{dat}とは重ならず、動詞に応じた使い分けがあると考えられる。直接話法と共に現れる動詞は英語の場合も限られており\citep[1024]{english}、双方の単語群の比較によってPijin語の英語からの影響を見て取れる可能性がある。なお、表\ref{tb:datl1}の8位であった\textit{talem}は\textit{dat}節だけでなく「\textit{olsem}+直接話法」という形を取ることができる(cf. (\ref{ex:talemolsem}))。
